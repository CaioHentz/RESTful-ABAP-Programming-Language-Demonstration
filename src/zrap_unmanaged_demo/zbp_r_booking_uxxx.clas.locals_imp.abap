...
CLASS lhc_Booking IMPLEMENTATION.
  ...
  DATA: messages TYPE /dmo/if_flight_legacy=>tt_message,
               legacy_booking_in  TYPE /dmo/booking,
               legacy_booking_x TYPE /dmo/s_booking_inx.

    LOOP AT entities ASSIGNING FIELD-SYMBOL(<booking_update>).

      legacy_booking_in = CORRESPONDING #( <booking_update> MAPPING FROM ENTITY ).

      legacy_booking_x-booking_id = <booking_update>-BookingID.
      legacy_booking_x-_intx      = CORRESPONDING #( <booking_update> MAPPING FROM ENTITY ).
      legacy_booking_x-action_code = /dmo/if_flight_legacy=>action_code-update.
      ...
      CALL FUNCTION '/DMO/FLIGHT_TRAVEL_UPDATE'
        EXPORTING
          ...
          it_booking  =
           VALUE /dmo/if_flight_legacy=>tt_booking_in( ( CORRESPONDING #( legacy_booking_in ) ) )
          it_bookingx = VALUE /dmo/if_flight_legacy=>tt_booking_inx( ( legacy_booking_x ) )
        IMPORTING
          et_messages = messages.

      IF messages IS INITIAL.
        APPEND VALUE #( travelid = <booking_update>-travelid
                       bookingid = legacy_booking_in-booking_id ) TO mapped-booking.
      ELSE.
        APPEND VALUE #( travelid = <booking_update>-travelid
                        bookingid = legacy_booking_in-booking_id ) TO failed-booking.
        LOOP AT messages INTO DATA(message).
          APPEND VALUE #( travelid = <booking_update>-travelid
                          bookingid = legacy_booking_in-booking_id
...
METHOD delete.
    ...
    LOOP AT keys ASSIGNING FIELD-SYMBOL(<key>).
      ...
      CALL FUNCTION '/DMO/FLIGHT_TRAVEL_UPDATE'
        EXPORTING
          ...
          it_booking  =
          VALUE  /dmo/if_flight_legacy=>tt_booking_in( ( booking_id = <key>-bookingid ) )
          it_bookingx =
          VALUE /dmo/if_flight_legacy=>tt_booking_inx( ( booking_id  = <key>-bookingid
  ...
  DATA: legacy_parent_entity_out TYPE /dmo/travel,
               legacy_bookings_out      TYPE /dmo/if_flight_legacy=>tt_booking,
    ...
    LOOP AT keys ASSIGNING FIELD-SYMBOL(<key_parent>)
      ...
      CALL FUNCTION '/DMO/FLIGHT_TRAVEL_READ'
        ...
        IMPORTING
          ...
          et_booking   = legacy_bookings_out
          et_messages  = messages.

      IF messages IS INITIAL.
        LOOP AT GROUP <key_parent> ASSIGNING FIELD-SYMBOL(<key>)
                                       GROUP BY <key>-%key.
          READ TABLE legacy_bookings_out INTO DATA(legacy_entity_out)
           WITH KEY travel_id  = <key>-%key-TravelID
                    booking_id = <key>-%key-BookingID .
          IF sy-subrc = 0.
            INSERT CORRESPONDING #( legacy_entity_out MAPPING TO ENTITY )
            INTO TABLE result.
          ELSE.
            INSERT
        ...
        LOOP AT GROUP <key_parent> ASSIGNING <key>.
          failed-booking = VALUE #(  BASE failed-booking
                   ...
                   ( %key-TravelID    = <key>-TravelID
                     ...
                     %fail-cause = COND #(
                                   WHEN msg-msgty = 'E'
                                   AND ( msg-msgno = '016' OR msg-msgno = '009' )
    ...
    LOOP AT keys_rba ASSIGNING FIELD-SYMBOL(<fs_travel>)
      ...
      ELSE.
        failed-booking = VALUE #(  BASE failed-booking
                          ...
                          ( %key-TravelID    = <fs_travel>-%key-TravelID
                            %key-BookingID   = <fs_travel>-%key-BookingID
                            %fail-cause      = COND #( WHEN msg-msgty = 'E'
                                   AND ( msg-msgno = '016' OR msg-msgno = '009' )
                                   THEN if_abap_behv=>cause-not_found
                                   ELSE if_abap_behv=>cause-unspecific ) ) ).
      ENDIF.
